...
function Palette() {
  const sw = [['Midnight glass', '#0B1430', 'linear-gradient(160deg,#27305E,#0B1430 60%,#050B1E)', 'Deep blue glazing that turns into a mirror of the sky.'],
    ['Copper fin', '#C9866A', 'linear-gradient(160deg,#F0C6A6,#C9866A 45%,#7E4A36)', 'Brushed composite fins, warm at noon and glowing at dusk.'],
    ['Dawn stone', '#ECE2D6', 'linear-gradient(160deg,#FFF6EC,#ECE2D6 50%,#CDBBA6)', 'A pale limestone base that holds the first light.'],
    ['Pine green', '#4F6B55', 'linear-gradient(160deg,#9FB38F,#4F6B55 50%,#1F2E25)', 'The courtyard planting, borrowed from the riverbank.'],
    ['Dusk rose', '#E8A184', 'linear-gradient(160deg,#F7D4C0,#E8A184 50%,#7D5A89)', 'The colour of the sky over the crown at 19:40.']];
  const [on, setOn] = useState(1);
  return <section className="v-palette v-pad" data-tone="dawn">
    <div className="v-palette-head"><Label>The composite palette</Label><Title lines={['Five colours', 'from one day.']} accent={[1]}/></div>
  ...
  return <>
    ...
    <section className="v-principles v-pad" data-tone="night">
      ...
      <Accordion items={[
        ...
        ['Materials you can touch.', 'Stone, timber and brushed copper, chosen to age well and feel warm under the hand.'],
...
function Map3D() {
  const pins = [['Embankment', '4 min', 68, 58, '/v3/silver/loc-1.webp'], ['School', '7 min', 30, 36, '/v3/era/joy-2.webp'], ['Central park', '10 min', 22, 68, '/v3/likova/env-bg-1.webp'],
    ['Concert hall', '12 min', 58, 22, '/v3/era/joy-c4.webp'], ['Market', '6 min', 42, 60, '/v3/era/joy-c1.webp'], ['Business district', '15 min', 82, 26, '/v3/likova/loc-slide-2.webp']];
  const [on, setOn] = useState(0), stage = useRef(), [tilt, setTilt] = useState({x: 52, z: -18});
  const move = e => { const r = stage.current.getBoundingClientRect(); setTilt({x: 48 + ((e.clientY - r.top) / r.height - .5) * -10, z: -18 + ((e.clientX - r.left) / r.width - .5) * 16}); };
  return <section className="v-map v-pad" data-tone="sand">
    <div className="v-map-copy">
      <Label>3D map / Location</Label>
      <Title lines={['Everything', 'on foot.']} accent={[1]}/>
      <p>Hover the map to tilt it, choose a pin to see the walk. Rings mark five, ten and fifteen minutes from the front door.</p>
      <ol>{pins.map(([n, t], i) => <li key={n}><button className={on === i ? 'on' : ''} onClick={() => setOn(i)} onMouseEnter={() => setOn(i)}><span>{String(i + 1).padStart(2, '0')}</span>{n}<em>{t}</em></button></li>)}</ol>
    </div>
    <div className="v-map-stage" ref={stage} onPointerMove={move} onPointerLeave={() => setTilt({x: 52, z: -18})}>
      <div className="v-map-plane" style={{transform: `rotateX(${tilt.x}deg) rotateZ(${tilt.z}deg)`}}>
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
          <defs><pattern id="streets" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M8 0H0V8" fill="none" stroke="currentColor" strokeWidth=".15" opacity=".35"/></pattern></defs>
          <rect width="100" height="100" fill="url(#streets)"/>
          <path d="M-5 78 C20 70 35 90 55 76 S85 50 105 58" fill="none" stroke="#7fa2c9" strokeWidth="7" opacity=".55"/>
          <path d="M10 60 Q22 55 30 72 T12 82Z" fill="#6E8B6A" opacity=".45"/>
          <path d="M-5 30 L105 42 M40 -5 L52 105 M-5 88 L60 20" stroke="currentColor" strokeWidth=".6" opacity=".4"/>
          <path d="M60 64 Q66 70 74 62" stroke="#c9866a" strokeWidth="1.2" fill="none"/>
        </svg>
        {[16, 30, 44].map(r => <i key={r} className="v-map-ring" style={{width: r * 2 + '%', height: r * 2 + '%'}}/>)}
        <span className="v-map-home"><b>AFRAH</b></span>
        {pins.map(([n, t, x, y], i) => <button key={n} className={`v-map-pin ${on === i ? 'on' : ''}`} style={{left: x + '%', top: y + '%'}} onClick={() => setOn(i)} aria-label={`${n}, ${t}`}><i/><span>{n}<em>{t}</em></span></button>)}
      </div>
      <figure className="v-map-card"><img src={pins[on][4]} alt=""/><figcaption><strong>{pins[on][0]}</strong><span>{pins[on][1]} from AFRAH</span></figcaption></figure>
    </div>
  </section>;
}
...
function SunStudy() {
  const ref = useRef(), near = useNear(ref), sun = useRef(.45), [v, setV] = useState(.45);
  const set = x => { sun.current = x; setV(x); };
  const minutes = Math.round(6 * 60 + v * 16 * 60), label = `${String(Math.floor(minutes / 60)).padStart(2, '0')}:${String(minutes % 60).padStart(2, '0')}`;
  const presets = [[Sunrise, 'Sunrise', .02], [Sun, 'Noon', .4], [Sunset, 'Dusk', .8], [Moon, 'Night', 1]];
  const jump = x => { const o = {t: sun.current}; gsap.to(o, {t: x, duration: 1.2, ease: 'power2.inOut', onUpdate: () => set(o.t)}); };
  return <section className="v-sun" ref={ref} data-tone="sky">
    <div className="v-sun-stage">{near && <Suspense fallback={null}><Tower mode="sun" sun={sun} transparent={false}/></Suspense>}</div>
    <div className="v-sun-ui v-pad">
      <div className="v-sun-copy"><Label>Interactive sun study</Label><h2>Follow the<br/><span className="v-accent">light.</span></h2><p>Drag the slider to move the sun around the tower and watch the shadows swing across the courtyard. At night the windows light up one by one.</p></div>
      <div className="v-sun-controls">
        <span className="v-sun-time">{label}</span>
        <input type="range" min="0" max="1" step=".001" value={v} onChange={e => set(Number(e.target.value))} aria-label="Time of day"/>
...
function Facade() {
  const layers = [['Lighting', 'Warm LED lines trace the crown after dark.'], ['Copper fins', 'Vertical composite fins shade the glass.'], ['Glazing', 'Triple glazing, floor to ceiling.'], ['Stone frame', 'Limestone slabs mark every floor.']];
...
export function Architecture() {
  ...
  return <>
    ...
    <section className="v-silhouette v-pad" data-tone="dusk">
      ...
      <div><Label>The silhouette</Label><Title lines={['Stepped,', 'lit, lifted.']} accent={[1]}/><p>The tower narrows in four steps as it rises — at the fourteenth, twentieth and twenty-fourth floors — ending in a crown of copper arches that lights up at dusk.</p>
...
function Materials() {
  const m = [['Stone', 'Honed limestone, cool and calm.', 'radial-gradient(circle at 30% 20%,#fff7ee,#e3d6c6 45%,#bfae99)'], ['Walnut', 'Oiled timber with a warm grain.', 'repeating-linear-gradient(100deg,#5b3a28 0 6px,#6d4631 6px 11px,#4b2f21 11px 15px)'],
    ['Bronze', 'Brushed metal that catches the light.', 'linear-gradient(120deg,#6e4a34,#d7a07f 40%,#8c5c40 60%,#f0c6a6 80%,#6e4a34)'], ['Linen', 'Soft textiles, natural colour.', 'repeating-linear-gradient(0deg,#e9e1d4 0 2px,#ddd3c3 2px 3px)']];
...
export function Interiors() {
  ...
  return <>
    ...
    <section className="v-full" data-tone="midnight">
      <Img src="/v3/era/apart-bg.webp" alt="Copper pendant lamps" parallax/>
...
function Selector() {
  ...
  return <section className="v-selector" ref={ref} data-tone="night">
    <div className="v-selector-stage">
      {near && <Suspense fallback={null}><Tower mode="select" selected={floor} onSelect={setFloor} onHover={setHover}/></Suspense>}
...
export function Residences() {
  ...
  const cols = [['Garden', 'Floors 1–7', 'Private gardens and direct courtyard access.', '/v3/silver/court-5.webp'], ['Courtyard', 'Floors 8–13', 'Views over the planting, light from two sides.', '/story/courtyard.webp'],
    ['Terrace', 'Floors 14–19', 'The first step of the tower: every home with a terrace.', '/v3/silver/terraces.webp'], ['Skyline', 'Floors 20–23', 'The river and the city skyline, framed.', '/v3/silver/gallery-2.webp'], ['Crown', 'Floors 24–26', 'Penthouses under the copper arches.', '/v3/silver/gallery-5.webp']];
  return <>
...
export function Contact() {
  ...
  return <>
    ...
    <section className="v-full" data-tone="midnight">
      <Img src="/v3/era/int-deco.webp" alt="Sculptural stone and copper object in the gallery" parallax/>
