...
function MapPage() {
  return <section className="v-mappage" data-tone="white">
    <Suspense fallback={null}><MapView/></Suspense>
...
const pages = {'/': Home, '/vision': Vision, '/landscape': Landscape, '/neighbourhood': Neighbourhood, '/architecture': Architecture,
  ...
  ...(Lab ? {'/dev/3d-lab': () => <Suspense fallback={null}><Lab/></Suspense>, '/dev/capture': () => <div className="v-capture"><Suspense fallback={null}><AfrahCapture mode="flight"/></Suspense></div>} : {})};
const AfrahCapture = import.meta.env.DEV ? lazy(() => import('./three/AfrahView')) : null;
...
function Loader({done}) {
  ...
  // v5: the percentage is the 3D engine's real loading progress; it completes once the hero building is on screen.
  useEffect(() => {
    let target = 4, shown = 0, alive = true, finished = false, raf;
    const onP = e => { target = Math.max(target, Math.min(100, Math.round(e.detail * 100))); };
    const onReady = e => { if (e.detail === 'hero') target = 100; };
    addEventListener('afrah:progress', onP); addEventListener('afrah:ready', onReady);
    document.fonts.ready.then(() => { target = Math.max(target, 12); });
    const safety = setTimeout(() => { target = 100; }, location.pathname === '/' ? 15000 : 2500);
    const tick = () => {
      raf = requestAnimationFrame(tick);
      shown += (target - shown) * .08;
      if (alive) setPct(Math.min(100, Math.round(shown + .4)));
      if (!finished && shown > 99.4) { finished = true; gsap.to(ref.current, {opacity: 0, duration: .9, delay: .3, ease: 'power2.inOut', onComplete: done}); }
    };
    tick();
    return () => { alive = false; cancelAnimationFrame(raf); clearTimeout(safety); removeEventListener('afrah:progress', onP); removeEventListener('afrah:ready', onReady); };
  }, [done]);
  return <div className="v-loader" ref={ref}>
    <svg className="v-arcs" viewBox="0 0 1200 800" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
      <defs>
        <linearGradient id="glint" gradientUnits="userSpaceOnUse" x1="-300" y1="0" x2="300" y2="220"><stop offset="0" stopColor="#2d2416"/><stop offset=".44" stopColor="#2d2416"/><stop offset=".5" stopColor="#f6d894"/><stop offset=".56" stopColor="#2d2416"/><stop offset="1" stopColor="#2d2416"/><animateTransform attributeName="gradientTransform" type="translate" values="-400 -60; 1600 160" dur="3.4s" repeatCount="indefinite"/></linearGradient>
    ...
    <div className="v-loader-foot v-pad"><Label>{pct < 55 ? 'Building the tower' : pct < 100 ? 'Lighting the city' : 'Welcome home'}</Label><span className="v-loader-pct">{String(pct).padStart(3, '0')}</span><button onClick={done}>Skip</button></div>
    <div className="v-loader-bar" style={{transform: `scaleX(${pct / 100})`}}/>
  </div>;
}
...
export default function App() {
  ...
  const [intro, setIntro] = useState(() => { try { return clean(location.pathname) === '/' && !sessionStorage.getItem('afrah-v5-intro') && !matchMedia('(prefers-reduced-motion: reduce)').matches; } catch { return false; } });
  ...
  const finishIntro = useCallback(() => { setIntro(false); try { sessionStorage.setItem('afrah-v5-intro', '1'); } catch {} }, []);
